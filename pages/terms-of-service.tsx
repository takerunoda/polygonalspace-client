...
export default function TermsOfService({headObject} : HeadProps) {
        ...
        <div className="privacyOuter">
          <h1 className="h1"><Language jp={`利用規約`} en={`Terms of Service`} /></h1>
              ...
              <h2 className="h2">
                ...
                <p className="privacyParagraph"><Language jp={<><span className="text-blue-500 cursor-pointer hover:underline font-bold"><a href={urlNASAMedia} target="_blank">NASAメディア利用規定</a></span>には、NASAの画像データ等は、当該規定を遵守したうえで、教育・情報目的のためにWEBページ等に掲載できる旨が記載されています。</>} en={<>According to <span className="text-blue-500 cursor-pointer hover:underline font-bold"><a href={urlNASAMedia} target="_blank">NASA Media Usage Guidelines</a></span>, NASA content including images, audio, video, and computer files may be used for educational or informational purposes without needing explicit permission, subject to compliance with these guidelines.</>} /></p>
                ...
                <p className="privacyParagraph">
                  <Language jp={<>ユーザーは、当サイトがNASAから取得・表示する画像・テキストデータを利用することができます (以下、「データ利用」と表記します)。ただし、データ利用する場合は、<span className="text-blue-500 cursor-pointer hover:underline font-bold"><a href={urlNASAMedia} target="_blank">NASAメディア利用規定</a></span>をはじめとした、NASAが規定するデータ利用についての諸規定を遵守する必要があります。</>} en={<>When you use the image and text data retrieved from NASA in this website, you should comply with the NASA guidelines including <span className="text-blue-500 cursor-pointer hover:underline font-bold"><a href={urlNASAMedia} target="_blank"> NASA Media Usage Guidelines</a></span> regarding usage of data provided by NASA.</>} />
                </p>
              </div>
              <div className="privacyH2">
              <h2 className="h2">
