...
const Navbar =() => {      
    ...
    const { spMenuState, setSpMenuState, pathName } = pageContext;
    const { setUserBookmark } = bookmarkContext;
    const {userEmail, setUserEmail, setUserId, setAccessToken, setIsLoggedin, isLoggedin, userStatus, setUserStatus, loginType, setLoginType} = userContext
    const router = useRouter()

   const handleLogoutInNavbar = async () => {
               await logout({setUserEmail, setAccessToken, setUserId, setIsLoggedin, setUserStatus, setLoginType, setUserBookmark
               })
               router.push("/login")
            }   
   const handleToggle = () => {
    ...
    return (
      <div className={`w-full mx-auto ${!pathName.includes("-details") && "fixed"} sm:relative sm-bg-white sm:opacity-100 z-40`}> 
      <nav id="header" className={`w-full sm:w-11/12 mx-auto z-10 top-0 bg-white sm:border-b border-gray-300 opacity-100 
      ${spMenuState && "pb-96 d:pb-0 "}
      `}>
        <div className="flex justify-between py-4 pb-2">
               ...
               <Link href="/">
                  <div onClick={() => setSpMenuState(false)} className="mr-3 sm:ml-10 items-center sm:px-5 py-5 sm:py-3 inline-block cursor-pointer hover:opacity-70">
                        <a className="font-Ubuntu text-2xl sm:text-3xl text-purple-600 no-underline hover:no-underline ">
                           <img src="/siteTitle.png" className={`w-36 sm:w-72`}/>
                        </a>
                  </div>
               </Link>
         {!isLoggedin && <div className="block md:hidden text-green-700 w-24 md:w-48 -right-0 ml-auto text-center mr-3 xs:mr-10">
            ...
            <div className="w-16 -right-0 ml-auto mt-0 md:mt-2">
               <AuthController logoutInNavbar={handleLogoutInNavbar}/>
            </div>
         </div>}
        ...
        <div className={`m-0 w-0 md:w-auto ${!isLoggedin && "hidden"} md:flex justify-center ml-auto`}>
          ...
          <div className="mx-5 mt-1 text-center text-blue-500 font-bold">
              <div className="w-56 hidden md:block mb-2 text-base mr-2 text-center">
               ...
               {/* {userStatus !== "guest" && !isLoggedin && (<p className="mt-8"></p>)}    */}
               {userStatus === "guest" && <Language jp={"ゲスト"} en={"Guest"}/>}                         
              </div>
               ...
               <ul className="sm:flex sm:flex-wrap items-center pl-12 sm:pl-0">
                  ...
                  <li className="navbarItem">
                     ...
                     <Link href="/shared-bookmarks">
                         <div className="inline-flex cursor-pointer">
                           <div className={`text-2xl mr-2 ${pathName === "/shared-bookmarks" ? "text-purple-500 no-underline  ": "text-blue-500"}`}>
                              <BiShareAlt />
                           </div>
                           <a className={`inline-block ${pathName === "/shared-bookmarks" ? "text-purple-500 underline font-bold sm:font-normal": "text-gray-800  hover:text-purple-500 hover:no-underline sm:hover:underline"}`}>
                  ...
                  <li className="navbarItem">
                     ...
                     <Link href="/posts">
                         <div className="inline-flex cursor-pointer">
                           <div className={`text-2xl mr-2 ${pathName === "/posts" ? "text-purple-500 no-underline  ": "text-blue-500"}`}>
                              <HiPencilAlt />
                           </div>
                           <a className={`inline-block ${pathName === "/posts" ? "text-purple-500 underline font-bold sm:font-normal": "text-gray-800  hover:text-purple-500 hover:no-underline sm:hover:underline"}`}>
